...
export default function TimePicker({ state, onChange, formattingIgnoreList }) {
  const {
    ...
    meridiems = useMemo(() => new DateObject({ calendar, locale }).meridiems, [
      calendar,
      locale,
    ]),
    mustShowTimePicker =
      (timePicker || onlyTimePicker) && !state.multiple && !state.range;
