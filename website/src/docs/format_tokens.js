...
export default function (translate, language) {
  ...
  const format = array => array.map(string => string.replace(
    /[0-9]/g,
    w => date.digits[w]
  )).join(separator)
  ...
  const tokens = {
    ...
    jsx: <table>
      ...
      <tbody>
        ...
        <tr>
          <td>dddd</td>
          <td>{[0, 1, 2].map(index => weekDays[index].name).concat("...").join(separator)}</td>
          <td>{translate("week day name")}</td>
          <td>{translate("format")}</td>
        </tr>
        <tr>
          <td>ddd</td>
          <td>{[0, 1, 2].map(index => weekDays[index].shortName).concat("...").join(separator)}</td>
