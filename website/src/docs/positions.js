...
export default function (translate, language) {
  ...
  const points = {
    ...
    description: <ol>
      {(translate("important_points") || []).map((point, index) => <li key={index}>{point}</li>)}
    </ol>
  }
  ...
  >
    <label>
      ...
      >${["top", "bottom", "left", "right"].map((position) => `
        <option value="${position}">${translate(position)}</option>`).join("")}
      </select>
    </label>
    <label>
      <input
        ...
        <label>
          ...
          >
            {["top", "bottom", "left", "right"].map((position, index) => {
              return (
                <option key={index} value={position}>{translate(position)}</option>
              )
            })}
          </select>
        </label>
