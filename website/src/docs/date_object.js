...
export default function (translate, language, otherProps, codeEnd, Code) {
  const description = {
    title: "Descriptions",
...
console.log(name, shortName, number, index); //Tuesday Tue 3 2`}
      ...
      <Code title="gregorian_7">{`date.toFirstOfMonth()
    ...
    description: <div>
      ...
      <ul>
        <li>
          ...
          <ol>
            <li><p>{translate("string_3")}</p></li>
            <li><div dangerouslySetInnerHTML={{ __html: `<p>${translate("string_4")}</p>` }} /></li>
          </ol>
...
console.log(date.format("dddd DD MMMM${language === "en" ? "" : " سال"} YYYY")) //${language === "en" ? "Friday 22 October 2021" : "چهارشنبه ۲۲ دی سال ۱۴۰۰"}`
  }

  const unix = {
    title: "New DateObject from Unix Timestamp",
    description: <div>
      {<div dangerouslySetInnerHTML={{ __html: `<p>${translate("unix_1")}</p>` }} />}
...
console.log(date.format()) //${language === "en" ? "2021/03/02" : "1399/12/12"}`}
      </Code>
      <Code title="unix_4">
